						...
						<div class="portlet box blue">
							...
							<div class="portlet-body">
								...
								<form role="form" action="edit" method="post"
									...
									<input value="${user.uid }" hidden="" name="id" />
										<label>Username:</label> <input value="${user.user }" class="form-control"
											placeholder="please enter category Name" name="user" />
									</div>
									<div class="form-group">
										<label>Password:</label> <input value="${user.pass }" type="password"
											class="form-control" placeholder="please enter category Name"
											name="pass" />
									</div>
									<div class="form-group">
										<label>IsSeller:</label>
										<c:if test="${user.isSell==1 }">
										<br>Có <input checked="checked" type="radio"
											value="1" name="isSeller" /> Không <input
											type="radio" value="0" name="isSeller" />
										</c:if>
										<c:if test="${user.isSell==0 }">
										<br>Có <input type="radio"
											value="1" name="isSeller" /> Không <input checked="checked"
											type="radio" value="0" name="isSeller" />
										</c:if>
									</div>
									<div class="form-group">
										...
										<br>Có <input checked="checked" type="radio"
											value="1" name="isAdmin" /> Không <input
											type="radio" value="0" name="isAdmin" />
										</c:if>
										<c:if test="${user.isAdmin==0 }">
										<br>Có <input type="radio"
											value="1" name="isAdmin" /> Không <input checked="checked"
											type="radio" value="0" name="isAdmin" />
										</c:if>
									</div>
